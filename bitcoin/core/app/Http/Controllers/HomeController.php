...
class HomeController extends Controller
{
	...
	{
		if(Auth::user()->tfver == 1 && Auth::user()->status == 0 && Auth::user()->tauth == 0)
		{
			return redirect()->route('user-dashboard');
		}
		elseif(Auth::user()->tauth == 1 && Auth::user()->tfver == 0)
		{
			$data['page_title'] = "Verify Two Factor Authenticator";
	...
	public function verify2fa( Request $request)
	{
		...
		if ($oneCode == $userCode) { 
         ...
         public function getKnowledge(){
            $data['page_title'] = "Knowledge page";
            $data['news']= \App\News::orderBy('updated_at','DESC')
                     ->paginate(5);
            return view('home.knowledge', $data);
            
        }

        public function getPriceChart(){
            $data['page_title'] = "BITCOIN PRICE CHART";
            
            return view('home.bitcoin-price-chart', $data);
